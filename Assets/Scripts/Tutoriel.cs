...
public class Tutoriel : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (count == 12 && pnj1.GetComponent<WearMask>().IsMasked() && pnj2.GetComponent<WearMask>().IsMasked() && pnj3.GetComponent<WearMask>().IsMasked() && !Gun.GetComponent<GunBehaviour>().isFiring())
        {
            Compris();
        }
        ...
        if (count == 14 && Input.GetMouseButtonDown(0)) //temporaire, à remplacer quand la grenade fera vraiment qq chose
        {
            StartCoroutine(GrenadeLancee());
        }
        if (count == 16 && PlayerPrefs.GetInt("MunTourelle", 0) == 0)
        {
            PlayerPrefs.SetInt("MunTourelle", 1);
        }
        if (count == 16 && pnj7.GetComponent<WearMask>().IsMasked() && pnj8.GetComponent<WearMask>().IsMasked() && pnj9.GetComponent<WearMask>().IsMasked())
        {
            Compris();
        }
        sourceMusiques.volume = (float)PlayerPrefs.GetInt("VolumeMusiques") / 400;
